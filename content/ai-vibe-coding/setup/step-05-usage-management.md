...
이 문서는 Claude Code를 요금제 기반으로 사용할 때, 사용량 제한이 어떻게 동작하고 어떻게 확인하는지를 다룹니다.

구체적인 요금, 한도 수치, 화면 구성은 서비스 정책에 따라 계속 바뀔 수 있으므로, 이 문서는 변하지 않는 원리를 중심으로 설명합니다. 정확한 최신 수치는 항상 공식 설정 페이지에서 직접 확인합니다.
