...
HTML로 문서의 뼈대와 내용을 만들었다면, 이제 그 위에 색과 여백, 배치를 입힐 차례다. CSS(Cascading Style Sheets)는 HTML 요소가 화면에 어떻게 보일지 결정하는 언어다. 이 문서에서는 CSS 코드를 어디에 작성하고, 어떤 형태로 써야 하며, 여러 스타일이 충돌할 때 브라우저가 무엇을 기준으로 우선순위를 정하는지를 다룬다. 구체적인 선택자 문법과 색상·폰트 속성은 다음 문서들에서 이어서 다룬다.
