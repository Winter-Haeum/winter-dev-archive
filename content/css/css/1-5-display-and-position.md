...
[[1-4-box-model|이전 문서]]에서 요소 하나의 크기와 여백을 다뤘다면, 이 문서에서는 그 요소들을 화면 어디에 어떤 방식으로 놓을지를 다룬다. display는 요소가 "어떻게 표시되는지"를, position은 "어디에 위치하는지"를 결정한다.
