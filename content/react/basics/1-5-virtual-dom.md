...
강의 대시보드에서 강의 하나의 수강 상태를 "진행 중"에서 "완료"로 바꾼다고 해봅시다. 화면에서 바뀌어야 하는 부분은 그 강의 카드 하나뿐입니다. 하지만 어떤 부분이 바뀌었는지, 무엇을 다시 그려야 하는지를 매번 직접 계산해서 지정하는 것은 번거롭고 실수하기 쉽습니다.
...
state가 바뀌면 컴포넌트 함수가 다시 실행되어 **새로운 UI 구조**를 계산합니다. 이 계산 결과는 실제 화면이 아니라, 화면 구조를 표현한 가벼운 JavaScript 객체입니다. 이것이 Virtual DOM입니다.
