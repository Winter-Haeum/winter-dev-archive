...
<div class="wda-check-note">
  <ul>
    <li><strong>reducer(state, action) → newState</strong> 함수가 상태 변경 로직을 전담하고, 컴포넌트는 dispatch로 호출만 한다.</li>
    <li>Action은 객체이며 <strong>type(필수)</strong>으로 행위를 식별하고, 필요하면 <strong>payload(선택)</strong>로 데이터를 함께 담는다. 예: <code>{ type: 'ADD', payload: '공부하기' }</code></li>
    <li><strong>useState</strong>는 숫자·문자열 같은 단순 값에, <strong>useReducer</strong>는 여러 하위 값이 얽힌 복잡한 로직에 적합하다.</li>
    <li>Reducer는 반드시 <strong>순수 함수</strong>여야 한다 (같은 입력 → 같은 출력, API 호출·외부 변수 수정 금지).</li>
    <li>상태는 <strong>불변성을 유지</strong>해야 한다. push/splice 대신 spread(...), map, filter로 새 배열·객체를 반환한다.</li>
  </ul>
</div>
...
<div class="wda-mistake-notes">
  <div class="wda-mistake-note">
    <div class="wda-mistake-wrong">오해: (state, action) => newState 형태의 함수 이름이 헷갈린다?</div>
    <div class="wda-mistake-right">정답: <strong>Reducer(리듀서)</strong>다. 이전 상태와 액션을 받아 새로운 상태를 반환하는 순수 함수다.</div>
  </div>
  <div class="wda-mistake-note">
    <div class="wda-mistake-wrong">오해: 리듀서에게 상태 변경을 요청(action 전달)할 때 쓰는 함수 이름이 헷갈린다?</div>
    <div class="wda-mistake-right">정답: <strong>dispatch(디스패치)</strong>다. dispatch(action) 형태로 호출해 리듀서를 실행시킨다.</div>
  </div>
  <div class="wda-mistake-note">
    <div class="wda-mistake-wrong">오해: dispatch 함수는 매 렌더링마다 새로 생성되어 의존성 배열에 꼭 넣어야 한다?</div>
    <div class="wda-mistake-right">정답: dispatch는 참조값이 <strong>항상 안정적(Stable)</strong>이라 useEffect·useCallback 의존성 배열에 넣지 않아도 안전하다.</div>
  </div>
  <div class="wda-mistake-note">
    <div class="wda-mistake-wrong">오해: Date.now()나 Math.random() 같은 값은 reducer 안에서 만들어도 된다?</div>
    <div class="wda-mistake-right">정답: reducer의 순수성을 지키기 위해, 이런 비순수 로직은 <strong>action을 만드는 시점(reducer 밖)</strong>에서 처리하는 것이 좋다.</div>
  </div>
  <div class="wda-mistake-note">
...
<div class="wda-flip-deck">
  <div class="wda-flip-card">
    ...
    <div class="wda-flip-back">(state, action) => newState 형태로 상태 변경 로직을 전담하는 순수 함수다.</div>
  </div>
  <div class="wda-flip-card">
    <div class="wda-flip-front">dispatch란?</div>
    <div class="wda-flip-back">action을 reducer에게 전달해 상태 변경을 요청하는 함수다.</div>
  </div>
  <div class="wda-flip-card">
    <div class="wda-flip-front">Action 객체의 필수 속성은?</div>
    <div class="wda-flip-back">type이며, 필요하면 payload로 추가 데이터를 함께 담는다.</div>
  </div>
  <div class="wda-flip-card">
    <div class="wda-flip-front">useState와 useReducer는 언제 구분하나?</div>
    <div class="wda-flip-back">단순한 값은 useState, 여러 하위 값이 얽힌 복잡한 로직은 useReducer를 사용한다.</div>
  </div>
  <div class="wda-flip-card">
    <div class="wda-flip-front">dispatch를 의존성 배열에 꼭 넣어야 하나?</div>
    <div class="wda-flip-back">아니다. dispatch는 참조가 항상 안정적이므로 넣지 않아도 안전하다.</div>
  </div>
  <div class="wda-flip-card">
    <div class="wda-flip-front">Reducer가 순수 함수여야 하는 이유는?</div>
    <div class="wda-flip-back">같은 입력에 항상 같은 출력을 보장해 예측 가능해야 하기 때문이며, API 호출 같은 side effect는 금지된다.</div>
  </div>
  <div class="wda-flip-card">
    <div class="wda-flip-front">state를 push/splice로 직접 수정하면 어떻게 되나?</div>
    <div class="wda-flip-back">리액트가 변경을 감지하지 못해 화면이 갱신되지 않는다. 반드시 새 배열·객체를 반환해야 한다.</div>
  </div>
</div>
