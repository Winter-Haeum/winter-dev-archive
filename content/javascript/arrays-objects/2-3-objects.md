...
<div class="wda-goal" style="position:relative;overflow:visible;padding-top:14px;">
  ...
  🛠️ <strong>메서드</strong> — 객체 내부의 함수와 this 키워드를 알게됩니다.
</div>
...
<div class="wda-callout wda-cs">
  ...
  <ul>
    ...
    <li><strong>성능</strong> : 자바스크립트 엔진은 리터럴 방식을 최적화하도록 설계되어 있어 생성자 방식보다 더 효율적입니다.</li>
...
// (참고: 숫자 형태의 키는 실제로는 항상 가장 먼저 출력됩니다)
```
...
<div class="wda-callout wda-ci">
  ...
  <ul>
    ...
    <li><strong>엄격한 순회</strong> : 따라서 의도치 않은 부모 객체의 메서드가 반복문에 포함되는 것을 막으려면, 위 코드처럼 반드시 <code>if (obj.hasOwnProperty(key))</code> 조건을 걸어 필터링해야 합니다. 이것이 가장 안전한 객체 순회 방식입니다.</li>
  </ul>
</div>
...
- **배열에서 for...in 사용 시 주의** : 배열에 `for...in`을 사용하면 요소의 값이 아닌 '인덱스(번호)'가 문자열 형태로 반환되며, 순서가 보장되지 않거나 의도치 않은 프로퍼티까지 순회할 수 있어 권장되지 않습니다.
...
- **권장 사항** : 최신 환경이라면 `structuredClone()`을 사용하는 것이 더 빠르고 정확하며 현대적인 표준 방식입니다.
...
- **화살표 함수의 한계** : 화살표 함수는 `this`를 상위 스코프(보통 전역 객체나 외부 함수)에서 상속받기 때문에, 객체의 프로퍼티에 접근해야 하는 메서드 용도로는 사용할 수 없습니다.
...
| **화살표 함수** | **사용 금지 (`❌`)** | 자체 `this`가 없어 객체 내부 데이터 접근이 불가능함 |
