...
[[2-1-analyze-webpage-structure|이전 문서]]에서 웹페이지를 눈으로 뜯어보는 분석법을 익혔다면, 이 문서는 그 분석 결과를 실제 HTML 구조로 옮기는 "설계" 단계를 다룹니다. 분석이 "읽는 연습"이라면, 블록 단위 사고는 "쓰는 연습"이다. 화면을 상자들의 조합으로 바라보고, 그 상자를 그대로 코드로 옮기는 훈련을 한다.
