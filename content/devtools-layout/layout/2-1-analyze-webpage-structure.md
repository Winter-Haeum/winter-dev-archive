...
CSS와 DevTools 사용법을 익혔다면, 다음 단계는 그 도구로 "실제 화면을 읽어내는 눈"을 기르는 것입니다. 이 문서는 눈앞의 웹페이지를 보고 "이 화면은 어떤 구조로 짜여 있을까"를 순서대로 뜯어보는 분석 방법을 다룹니다. 코드를 작성하는 문서가 아니라, 남이 만든 화면을 읽는 관점을 훈련하는 문서입니다.
