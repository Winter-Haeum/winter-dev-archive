...
Git 챕터의 첫 문서로, 코드 명령어보다 "왜 Git을 쓰는가"에 집중한다. 버전 관리가 없을 때 생기는 문제, Git이 만들어진 배경, 그리고 Git이 속한 분산형 버전관리시스템(DVCS)이 중앙집중형 방식과 어떻게 다른지를 다룬다. 실제 명령어와 Git 내부 구조는 다음 문서인 3-1-2, 3-1-3에서 이어서 다룬다.
